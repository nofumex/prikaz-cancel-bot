...
Этап `Консультация-НО` и переход в воронку `Отдел продаж` обнаруживаются
фоновым incremental polling worker раз в 5 секунд; webhook amoCRM для этого не требуется.
Worker читает только события `lead_status_changed` после сохранённого cursor с небольшим
overlap. События сначала записываются в durable inbox и лишь затем обрабатываются,
поэтому временная ошибка одной сделки не блокирует cursor и не теряет изменение.
Полная выгрузка воронок не выполняется. При запуске один раз параллельно запускается
ограниченная reconciliation только локально известных активных CRM-сделок.
