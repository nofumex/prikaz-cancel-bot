...
async def _send(client: MaxBotClient, event: IncomingEvent, text: str, keyboard=None) -> dict:
    return await client.send_message(chat_id=event.chat_id, text=text, keyboard=keyboard)
...
async def _edit_or_send(client: MaxBotClient, event: IncomingEvent, text: str, keyboard=None) -> None:
    if event.message_id:
        try:
            ...
            logger.info('MAX message edit unavailable message_id=%s; replacing message', event.message_id)
            try:
                await client.delete_message(event.message_id)
            except Exception:
                logger.info('MAX message delete unavailable message_id=%s', event.message_id)
...
async def _notify_consultation_staff_max(client: MaxBotClient, settings: Settings, text: str, *, test_mode: bool) -> None:
    notification_ids = consultation_notification_ids(settings, test_mode=test_mode)
    if settings.telegram_bot_token:
        from aiogram import Bot

        telegram_bot = Bot(settings.telegram_bot_token)
        try:
            for staff_id in notification_ids:
                try:
    ...
    for staff_id in notification_ids:
        try:
            ...
            logger.exception('Failed to notify MAX staff about consultation staff_id=%s', staff_id)


async def _submit_max_consultation(client: MaxBotClient, event: IncomingEvent, session, settings: Settings, user: User, *, test_mode: bool) -> None:
    await submit_consultation(
        session, settings, user, chat_id=event.chat_id,
        notify=lambda text: _notify_consultation_staff_max(client, settings, text, test_mode=test_mode),
    )
...
async def handle_update(client: MaxBotClient, event: IncomingEvent, settings: Settings) -> None:
    ...
    async with SessionLocal() as session:
        ...
        if data == "profile:show":
            ...
            return
        if data == 'case:my:noop':
            return
        if data == "case:my" or (data and data.startswith('case:my:')):
            cases = await generated_cases(session, user.id)
            page_size = 5
            requested_page = int(data.rsplit(':', 1)[-1]) if data.count(':') == 2 else 0
        ...
        if data == "case:new":
            ...
            case = await get_or_create_active_case(session, user, chat_id=event.chat_id, force_new=True)
        ...
        if data and data.startswith('paid:field:'):
            ...
            await _edit_or_send(client, event, f'Текущее значение поля <b>{FIELD_LABELS.get(field, field)}</b>:\n<code>{h(current)}</code>\n\nНапишите верное значение.')
            return
        if data and data.startswith('paid:review'):
            parts = data.split(':')
        ...
        if data and data.startswith('paid:regenerate'):
            ...
            if paid_regeneration_requires_new_date(case):
                ...
                return
            await _edit_or_send(client, event, '🔄 Перегенерирую заявление с исправленными данными...')
            try:
            ...
            return
        if data and data.startswith('paid:date:'):
            case = await generated_case_for_user(session, user.id, int(data.rsplit(':', 1)[-1]))
            if not case:
                await _send(client, event, 'Заявление не найдено.')
                return
            await _set_state(session, event, STATE_PAID_DATE, {'case_id': case.id})
            await _edit_or_send(client, event, manual_received_date_prompt_text())
            return
        ...
        if event.update_type == 'message_callback':
            ...
            if data in {'consultation:request', 'consultation:request:test'}:
                ...
                if user.phone and not test_mode:
                    await _submit_max_consultation(client, event, session, settings, user, test_mode=False)
                    await _send(client, event, CONSULTATION_ACCEPTED_TEXT)
                else:
                    await _set_state(session, event, STATE_CONSULTATION_PHONE, {'consultation_test_mode': test_mode})
        ...
        if current_state == STATE_PAID_FIELD and event.text:
            ...
            return
        if current_state == STATE_PAID_DATE and event.text:
            state_data = await _state_data(session, event)
            case = await session.get(Case, state_data['case_id'])
            received, error = validate_received_date(case, event.text)
            if error:
