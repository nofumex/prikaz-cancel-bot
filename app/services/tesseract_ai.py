...
RENDER_FIELD_DESCRIPTIONS = {
    "judge_name": (
        ...
        "Без префиксов «Судья:», «судья» или «Мировой судья»."
    ),
    "case_identifier": (
        "Только номер с символом № и УИД при наличии, например "
        "«№ 02-0987/94/2026, УИД 24MS0094-01-2026-001425-79». "
        "Без слов «Дело», «Производство», «по делу» и «по производству»."
    ),
    "debtor_full_name": (
        "Полное ФИО должника целиком в именительном падеже: фамилия, имя и отчество."
    ),
    "debtor_short_name": (
        "Краткое ФИО для подписи, сформированное только из render.debtor_full_name: "
        "фамилия в именительном падеже и инициалы."
    ),
    "order_facts_sentence": (
        "Готовое предложение, которое начинается с даты приказа словами, содержит суд и судью, "
        "точные слова «вынесен судебный приказ», взыскателя, должника, основание задолженности, "
        "период при его наличии, взысканные суммы и госпошлину при их наличии."
    ),
}
    ...
    "properties": {
        ...
        "render": {
            ...
            "properties": {
                name: {
                    "type": "string",
                    **({"description": RENDER_FIELD_DESCRIPTIONS[name]} if name in RENDER_FIELD_DESCRIPTIONS else {}),
                }
...
render.judge_name содержит ТОЛЬКО фамилию и инициалы, например «Кирюхин Н.О.»,
«Ларионова Е.Ф.» или «Миронов Е.К.». Не добавляй «Судья:», «судья» или
«Мировой судья»: renderer сам добавляет «Судья: ».

render.case_identifier содержит ТОЛЬКО номер с символом № и УИД при наличии:
«№ 02-0987/94/2026, УИД 24MS0094-01-2026-001425-79» или «№ 2-1252/2026».
Никогда не добавляй внутрь поля «Дело», «дело», «Производство»,
«производство», «по делу» или «по производству». Поле вставляется отдельно в
шапку и после готовой фразы «по делу/производству».

render.debtor_full_name всегда содержит полное ФИО целиком в именительном
падеже. Проверь именительный падеж фамилии, имени и отчества, не смешивай
падежи. render.debtor_short_name формируй строго из render.debtor_full_name:
фамилия в именительном падеже и инициалы. Не бери фамилию для подписи отдельно
из OCR-формы в другом падеже.

render.order_facts_sentence всегда начинается с даты судебного приказа словами,
содержит суд и судью, точные слова «вынесен судебный приказ», взыскателя и
должника. Базовая конструкция: «<дата> <суд и судья> вынесен судебный приказ
по заявлению <взыскатель> о взыскании с <должник в родительном падеже>
задолженности <основание, период и суммы>.» Обязательно укажи основание
задолженности, период, если он есть в приказе, а также взысканные суммы и
государственную пошлину, если они указаны. Допустима естественная адаптация,
но нельзя заменять вынесение приказа фразой «рассмотрено заявление», пропускать
должника или взыскателя либо писать только сведения о договоре без указания,
с кого взыскивается долг. Python вставит render без изменений.
